...
| システム                | 平均スコア       | トップスコア | 平均時間(ms) | 特性               |
| ----------------------- | ---------------- | ------------ | ------------ | ------------------ |
| **Naive RAG**     | 0.6763           | 0.7032       | 12.08        | ⚡最速・基本的     |
| **RAPTOR**        | 0.5689           | 0.7032       | 64.38        | 📊階層構造         |
| **ColBERT**       | **0.9565** | **0.9594** | 1652.33      | 🎯最高精度         |
| **Selective RAG** | 0.7463           | 0.7662       | 682.65       | ⚖️適応的バランス |

![4システム比較](output/comparison_200q_4systems.png)

### 主要な発見

#### 1. ColBERTの圧倒的優位性

- **精度**: 0.9565（他システムを41%~68%上回る）
- **課題**: 最も遅い（1652.33ms）
- **教訓**: 高精度が必須の場面では、速度を犠牲にしてでもColBERTを選ぶべき

#### 2. RAPTORの期待外れ

- **精度**: 0.5689（最下位、Naive RAGより低い）
- **速度**: Naiveの5.3倍遅い（64.38ms）
- **教訓**: このドメインでは階層化の恩恵なし。文書構造によっては有効だが、技術基準のような平坦な文書には不向き

#### 3. Naive RAGの実用性

- **精度**: 0.6763（基本的なタスクには十分）
- **速度**: 圧倒的に速い（12.08ms）
- **教訓**: リアルタイムチャットやプロトタイピングには最適。速度最優先ならNaive一択

#### 4. Selective RAGの成功

- **選択精度**: 96.0%（目標85%を大幅達成）
- **精度**: 0.7463（Naiveより+10.3%、ColBERTより-22.0%）
- **速度**: ColBERTの2.4倍高速（682.65ms）
- **選択分布**: ColBERT 54.0% / Naive 46.0%
...
1. 🥇 ColBERT: **0.9565** (トップスコア: 0.9594)
2. 🥈 Selective RAG: **0.7463** (トップスコア: 0.7662)
3. 🥉 Naive RAG: **0.6763** (トップスコア: 0.7032)
4. RAPTOR: **0.5689** (トップスコア: 0.7032)

#### 速度ランキング

1. 🥇 Naive RAG: **12.08ms**
2. 🥈 RAPTOR: **64.38ms**
3. 🥉 Selective RAG: **682.65ms**
4. ColBERT: **1652.33ms**
...
```
高精度・低速 ←→ 低精度・高速
ColBERT → Selective → Naive → RAPTOR
(0.9565)   (0.7463)   (0.6763)  (0.5689)
...
- **137倍の速度差**（Naive 12.08ms vs ColBERT 1652.33ms）
- **41%の精度差**（Naive 0.6763 vs ColBERT 0.9565）
- → 用途に応じた明確な選択基準が必要
...
- [� 4システム詳細レポート](output/REPORT_200Q_4SYSTEMS.txt)
- [� 4システム総合比較グラフ](output/comparison_200q_4systems.png)

---

**更新日**: 2025年11月11日 | **バージョン**: 1.2.0 | **比較実施日**: 2025年11月11日
