...
class RAPTORRAG:
    ...
    def _create_summary(self, node_ids: List[str]) -> str:
        """クラスタから要約を生成（LLM使用）"""
        # 子ノードのコンテンツを収集（橋梁診断ロジック準拠）
        texts = [self.nodes[nid].content for nid in node_ids[:10]]  # 最大10件に拡大
        combined_text = "\n\n".join(texts)  # 全文使用（切り詰めない）
        
        # Ollama使用
        if USE_OLLAMA and hasattr(self, 'use_ollama') and self.use_ollama:
            try:
                # 橋梁診断で成功したプロンプト形式
                prompt = f"""あなたは河川砂防ダムの技術専門家です。以下の技術文書を統合的に要約してください。

要約の際の重要ポイント:
1. 重要な技術用語・基準値・数値を必ず保持
2. 設計手法・管理手法の要点を明確化
3. 具体的な点検項目や評価基準を含める
4. 構造・地質・流域に関する情報を統合
...
統合要約（200-400字で簡潔に）:"""
                ...
                response = ollama.generate(
                    ...
                    options={
                        ...
                        'top_p': 0.9,
                        'num_predict': 400,  # 出力を拡大
                        'num_ctx': 4096  # コンテキスト拡大
                    },
                    keep_alive='10m'  # 長時間保持
                )
                
                summary = response['response'].strip()
                
                if summary and len(summary) > 50:  # 最小長を厳格化
                    return summary
